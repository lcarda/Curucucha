...
// Non-Membership
    ...
    pub fn commit_2(
        &self,
        p: &[E::ScalarField]
    ) -> Result<E::G2, KZGError> {
        if p.len() > self.max_d + 1 {
            return Err(KZGError::PolynomialTooLarge);
        }

        let mut commitment = E::G2::zero();

        for (i, &coeff) in p.iter().enumerate() {
            commitment += self.g2_pow[i] * coeff;
        }
    ...
    pub fn non_commit(
        ...
        // p(X) - v_0        
        let v_0_poly = vec![v_0];
        let poly_0 = subtract_polynomials::<E>(p, &v_0_poly); 

        // Commit to the resulting polynomial
        let commitment = self.commit(&poly_0)?;
    ...
    pub fn non_open(
        ...
        let (s, t) = match cgcd_polynomials::<E>(&poly_0, &zero_p)
        // Generate the commitments for S(X) and T(X)
        let proof_s = self.commit_2(&s)?;
        let proof_t = self.commit(&t)?;
    ...
    pub fn non_verify(
        ...
        let zero_p_com = self.commit_2(&zero_p)?;
        
        //e(P(tau),S(tau))+e(T(tau),Vanish(tau))==e(1,1)
        Ok(E::pairing(commitment,proof_s) + E::pairing(proof_t,zero_p_com) == E::pairing(self.g1_gen,self.g2_gen))
        }
...
// Counting 
    ...
    pub fn count_open(
        ...
        p: &[E::ScalarField],
        v_0:  &E::ScalarField,
        points_j: &[E::ScalarField], // Set of  w_j sush that P(w_j) = v0
        points_k: &[E::ScalarField], // Set of w such that P(w_k) != v0
    ) -> Result<(E::G1, E::G2, E::G1), KZGError> {   
        ...
        // Part of membership
        let quotient = divide_polynomials::<E>(&poly_0, &zero_polynomial::<E>(points_j))?;
        let quo = self.commit(&quotient);

        // Part of non-mebership
        let (s, t) = self.non_open(&poly_0, points_k, v_0)?;
    ...
    pub fn count_verify(
        &self,
        commitment: E::G1,   // C = non_commit(P(X)-v_0)
        proof: E::G1, // proof of q(t)
        proof_s: E::G2, // proof of non membership
        proof_t: E::G1, // proof of non membership
        points_j: &[E::ScalarField],
        points_k: &[E::ScalarField]
    ) -> Result<bool, KZGError> {

        let zero_p = zero_polynomial::<E>(points_j);
        let zero_p_com = self.commit_2(&zero_p)?;



        let check1 = E::pairing(commitment, self.g2_gen) == E::pairing(proof, zero_p_com);
        let check2 = self.non_verify(commitment, points_k, proof_s, proof_t)?;

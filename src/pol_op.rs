...
pub fn cgcd_polynomials<E: Pairing>(
    p: &[E::ScalarField],
    q: &[E::ScalarField],
) -> Result<(Vec<E::ScalarField>, Vec<E::ScalarField>), OperationError> {
    // Verifica si el polinomio q es cero para evitar la división por cero
    if q.is_empty() || q.iter().all(|&q| q.is_zero()) {
        return Err(OperationError::DivisionByZero);
    }

    // Convierte los polinomios de entrada a vectores para facilitar la manipulación
    let (mut r_old, mut r_new) = (p.to_vec(), q.to_vec());
    let (mut s_old, mut s_new) = (vec![E::ScalarField::one()], vec![E::ScalarField::zero()]);
    let (mut t_old, mut t_new) = (vec![E::ScalarField::zero()], vec![E::ScalarField::one()]);

    // Ejecuta el algoritmo del máximo común divisor de polinomios
    while r_new != vec![E::ScalarField::zero()] {
        // Divide r_old por r_new y obtiene el cociente (quo)
        let quo = match divide_polynomials::<E>(&r_old, &r_new) {
            Ok(q) => q,
            Err(e) => return Err(e),
        };

        // Clona los polinomios para usarlos como referencia en la siguiente iteración
        let r_aux = r_new.clone();
        let s_aux = s_new.clone();
        let t_aux = t_new.clone();

        // Multiplica los polinomios por el cociente obtenido
        let r_multi = multiply_polynomials::<E>(&quo, &r_aux);
        let s_multi = multiply_polynomials::<E>(&quo, &s_aux);
        let t_multi = multiply_polynomials::<E>(&quo, &t_aux);

        // Resta los resultados multiplicados del polinomio anterior
        r_new = subtract_polynomials::<E>(&r_old, &r_multi);
        s_new = subtract_polynomials::<E>(&s_old, &s_multi);
        t_new = subtract_polynomials::<E>(&t_old, &t_multi);

        // Actualiza los valores antiguos para la siguiente iteración
        r_old = r_aux;
        s_old = s_aux;
        t_old = t_aux;
    }

    // Retorna los coeficientes de Bézout y el máximo común divisor de los polinomios
    Ok((s_old, t_old))
}
